...
TEST(TestMathUtils, Test1)
{
  ...
  DDMPC::StandardScaler<double> standard_scaler(train_data);
  ...
  // Check train data
  {
    Eigen::RowVector3d mean = DDMPC::StandardScaler<double>::calcMean(standardized_train_data);
    Eigen::RowVector3d stddev = DDMPC::StandardScaler<double>::calcStddev(standardized_train_data, mean);
  ...
  // Check test data
  {
    Eigen::RowVector3d mean = DDMPC::StandardScaler<double>::calcMean(standardized_test_data);
    Eigen::RowVector3d stddev = DDMPC::StandardScaler<double>::calcStddev(standardized_test_data, mean);
